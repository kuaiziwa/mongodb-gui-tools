      ...
      <tbody>
        <template v-for="row in group.rows" :key="row">
          <tr valign="top" class="matrix-row" v-show="showThisRow(row)">
            <th scope="row">
              <span data-toggle="tooltip"  :class="{ tippy_content_interactive: row.interactive, tippy_content_no_interactive:!row.interactive}" :data-tippy-content="row.title"  v-html="row.name"></span>
            </th>

            <td v-for="cell in row.cells" :key="cell" :class="{ tippy_content_interactive: cell.interactive,tippy_content_no_interactive:!cell.interactive, fm_yes: cell.yes, fm_no:!cell.yes, value_is_string: cell.valueIsString }" :data-tippy-content="cell.title" v-html="cell.html"></td>
          </tr>
        </template>
      </tbody>
      </template>
